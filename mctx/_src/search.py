...
def simulate(
    ...
    def body_fun(state):
        ...
        action = action_selection_fn(action_selection_key, tree, node_index, state.depth)
...
def expand(
    ...
    tree = update_tree_node(tree, next_node_index, step.prior_logits, step.value, embedding)

    # Return updated tree topology.
    return tree.replace(
        children_index=batch_update(tree.children_index, next_node_index, parent_index, action),
        children_rewards=batch_update(tree.children_rewards, step.reward, parent_index, action),
        ...
        action_from_parent=batch_update(tree.action_from_parent, action, next_node_index),
    )
...
def backward(tree: Tree[T], leaf_index: chex.Numeric) -> Tree[T]:
    ...
    def body_fun(loop_state):
        ...
        tree = tree.replace(
            ...
            children_values=update(tree.children_values, children_values, parent, action),
            children_visits=update(tree.children_visits, children_counts, parent, action),
        )
