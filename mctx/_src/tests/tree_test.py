...
def _prepare_root(batch_size, num_actions):
    ...
    output = jax.vmap(functools.partial(_produce_prediction_output, num_actions=num_actions))(
        embedding
    )
...
def _prepare_recurrent_fn(num_actions, *, discount, zero_reward):
    ...
    def recurrent_fn(params, rng_key, action, embedding):
        ...
        embedding = jax.vmap(functools.partial(_fold_action_in, num_actions=num_actions))(
            embedding, action
        )
        output = jax.vmap(functools.partial(_produce_prediction_output, num_actions=num_actions))(
            embedding
...
def tree_to_pytree(tree: mctx.Tree, batch_i: int = 0):
    ...
    nodes = {}
    nodes[0] = _create_pynode(tree, batch_i, 0, prior=1.0, action=None, reward=None)
...
class TreeTest(parameterized.TestCase):
    ...
    @parameterized.named_parameters(
        ("muzero_norescale", "../mctx/_src/tests/test_data/muzero_tree.json"),
        (
            "muzero_qtransform",
