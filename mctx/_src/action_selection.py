...
def gumbel_muzero_root_action_selection(
    ...
    *,
    ...
    table = jnp.array(
        seq_halving.get_table_of_considered_visits(max_num_considered_actions, num_simulations)
    )
...
def _prepare_argmax_input(probs, visit_counts):
    ...
    to_argmax = probs - visit_counts / (1 + jnp.sum(visit_counts, keepdims=True, axis=-1))
    return to_argmax
