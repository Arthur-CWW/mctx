...
def _run_demo(rng_key: chex.PRNGKey) -> Tuple[chex.PRNGKey, DemoOutput]:
    ...
    prior_logits = jax.random.normal(logits_rng, shape=[batch_size, FLAGS.num_actions])
    ...
    policy_output = mctx.gumbel_muzero_policy(
        ...
        qtransform=functools.partial(
            mctx.qtransform_completed_by_mix_value, use_mixed_value=use_mixed_value
        ),
    )
    ...
    action_weights_policy_value = jnp.sum(policy_output.action_weights * qvalues, axis=-1)
...
def _make_bandit_recurrent_fn(qvalues):
    ...
    def recurrent_fn(params, rng_key, action, embedding):
        ...
        reward = jnp.where(embedding == 0, qvalues[jnp.arange(action.shape[0]), action], 0.0)
...
def main(_):
    ...
    for _ in range(FLAGS.num_runs):
        ...
        action_value_improvement = output.selected_action_value - output.prior_policy_action_value
        weights_value_improvement = output.action_weights_policy_value - output.prior_policy_value
        print(
